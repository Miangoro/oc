...
use App\Models\activarHologramasModelo_052;
use App\Models\empresa;
use App\Models\solicitudHolograma_052 as ModelsSolicitudHolograma;
...
class hologramasActivar_052 extends Controller
{
    public function find_hologramas_activar()
    {
        ...
        return view('hologramas.find_activar_hologramas-052_view', [
            'totalUser' => $userCount,
            'verified' => $verified,
    ...
    $query = activarHologramasModelo_052::when(['inspeccion', 'solicitudHolograma_052.marcas', 'solicitudHolograma_052.empresa'])
        ...
        if (!empty($searchValue)) {
            $query->where(function ($q) use ($searchValue) {
                ...
                $q->orWhereHas('solicitudHolograma_052', function ($sql) use ($searchValue) {
                    $sql->where('folio', 'LIKE', "%{$searchValue}%");
                });

                $q->orWhereHas('solicitudHolograma_052.marcas', function ($Marca) use ($searchValue) {
                    $Marca->where('marca', 'LIKE', "%{$searchValue}%");
                });
            });
        }

    $totalData = activarHologramasModelo_052::when($empresaId, function ($q) use ($empresaId) {
        $q->whereHas('solicitudHolograma_052.empresa', function ($q2) use ($empresaId) {
            $q2->where('id_empresa', $empresaId);
        });
    })->count();
        ...
        if ($datos->isNotEmpty()) {
            ...
            foreach ($datos as $dato) {
                ...
                for ($i = 0; $i < count($folios['folio_inicial']); $i++) {
                    $rangoFolios[] =
                    '<a target="_blank" href="/holograma/' . $numero_cliente . '-' .$dato->solicitudHolograma_052->tipo. $folioMarca.str_pad($folios['folio_inicial'][$i], 7, '0', STR_PAD_LEFT) . '">' .
                    $numero_cliente . '-'  .$dato->solicitudHolograma_052->tipo. $folioMarca. str_pad($folios['folio_inicial'][$i], 7, '0', STR_PAD_LEFT) .
                    '</a> a ' .
                    '<a target="_blank" href="/holograma/' . $numero_cliente . '-' .$dato->solicitudHolograma_052->tipo. $folioMarca. str_pad($folios['folio_final'][$i], 7, '0', STR_PAD_LEFT) . '">' .
                    $numero_cliente . '-' .$dato->solicitudHolograma_052->tipo. $folioMarca. str_pad($folios['folio_final'][$i], 7, '0', STR_PAD_LEFT) .
                    '</a>';
    ...
    public function activarHologramas(Request $request)
    {
        ...
        $activarHologramas = new activarHologramasModelo_052();
    ...
    {
        $loteEnvasado = new activarHologramasModelo_052();
    ...
    public function editActivados($id)
    {
        try {
            // Obtener el registro
            $activo = activarHologramasModelo_052::find($id);
    ...
    public function updateActivados(Request $request)
    {
        ...
        try {
            $loteEnvasado = activarHologramasModelo_052::findOrFail($request->input('id'));
