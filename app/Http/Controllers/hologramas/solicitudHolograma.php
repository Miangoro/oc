        ...
        if ($users->isNotEmpty()) {
            ...
            foreach ($users as $user) {
                ...
                $nestedData = [
                    ...
                    'folio_inicial' => '<a target="_blank" href="' . url('/hologramas-validacion/'.$numero_cliente.'-'.$user->marcas->folio.$user->tipo.str_pad($user->folio_inicial, 7, '0', STR_PAD_LEFT)) . '">' . $numero_cliente.'-'.$user->marcas->folio.$user->tipo.str_pad($user->folio_inicial, 7, '0', STR_PAD_LEFT) . '</a>',

                    'folio_final' => '<a target="_Blank" href="'.url('/hologramas-validacion/'.$numero_cliente.'-'.$user->marcas->folio.$user->tipo.str_pad($user->folio_final, 7, '0', STR_PAD_LEFT)).'">'.$numero_cliente.'-'.$user->marcas->folio.$user->tipo.str_pad($user->folio_final, 7, '0', STR_PAD_LEFT).'</a>',
