...
if (!empty($idHologramas) && is_array($idHologramas)) {
    // Tomar el primer elemento
    $primerHolo = reset($idHologramas); // equivalente a $idHologramas[0]
    $id_activacion = $primerHolo['id'] ?? null;
    ...
    if ($id_activacion) {
        ...
        $foliosTexto = [];
        foreach ($idHologramas as $rango) {
            $inicio = str_pad($rango['inicio'] ?? '', 7, '0', STR_PAD_LEFT);
            $final  = str_pad($rango['final'] ?? '', 7, '0', STR_PAD_LEFT);

            $prefijo = $no_cliente && $solic ? $no_cliente . '-' . $solic->tipo . $solic->marcas->folio : '';
            $foliosTexto[] = $prefijo ? "$prefijo$inicio - $prefijo$final" : "$inicio - $final";
        }
