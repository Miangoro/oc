...
@section('title', 'Bitácora Producto envasado sin etiqueta')
...
<option value="" selected>-- Selecciona la instalación --</option>
@forelse($instalacionesUsuario as $inst)
    <option value="{{ $inst->id_instalacion }}">
        {{ limpiarTipoPHP($inst->tipo) }} | {{ $inst->direccion_completa }}
    </option>
@empty
...
`;
    ...
    @vite(['resources/js/bitacora_producto_etiqueta.js'])
@endsection
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-header pb-0 mb-1">
            <h3 class="card-title mb-0">Producto envasado sin etiqueta</h3>
        </div>
    ...
    @include('_partials/_modals/modal-add-bitacoraProductoEtiqueta')
    @include('_partials._modals.modal-edit-bitacoraProductoEtiqueta')
