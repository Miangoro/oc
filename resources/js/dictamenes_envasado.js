      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            ...
            return (
              ...
              `<i data-id="` +
              $id +
              `" class="ri-file-pdf-2-fill text-danger ri-28px cursor-pointer pdfDictamen" data-bs-target="#mostrarPdf" data-bs-toggle="modal" data-bs-dismiss="modal"></i>`
            );
          }
        },
        {
...

        {
          ...
          render: function (data, type, full, meta) {
            ...
            if (puedeReexpedir) {
              acciones += `<a data-id="${full['id_dictamen_envasado']}" data-bs-toggle="modal" data-bs-target="#ModalReexpedir" class="dropdown-item waves-effect text-dark reexpedir"><i class="ri-file-edit-fill text-success"></i> Reexpedir/Cancelar</a>`;
            }
            if (puedeEliminar) {
              acciones += `<a data-id="${full['id_dictamen_envasado']}" class="dropdown-item waves-effect text-dark eliminar"><i class="ri-delete-bin-7-line ri-20px text-danger"></i> Eliminar</a>`;
            }
...
$(function () {
  ...
  $(document).ready(function () {
    ...
    function cargarDatosReexpedicion(id_dictamen) {
      ...
      //cargar los datos
      $.get('/dictamenes/envasado/' + id_dictamen + '/edit')
        .done(function (datos) {
