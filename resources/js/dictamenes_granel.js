      ...
      columnDefs: [
...

        {
          ...
          render: function (data, type, full, meta) {
            ...
            if (puedeReexpedir) {
              acciones += `<a data-id="${full['id_dictamen']}" data-bs-toggle="modal" data-bs-target="#ModalReexpedir" class="dropdown-item waves-effect text-black reexpedir"> <i class="ri-file-edit-fill text-success"></i> Reexpedir/Cancelar</a>`;
            }
            if (puedeEliminar) {
              acciones += `<a data-id="${full['id_dictamen']}" class="dropdown-item waves-effect text-dark eliminar"><i class="ri-delete-bin-7-line ri-20px text-danger"></i> Eliminar</a>`;
            }
