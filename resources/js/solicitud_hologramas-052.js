...
$(function () {
  ...
  if (dt_user_table.length) {
    var dt_user = dt_user_table.DataTable({
      ...
      serverSide: true,
      ajax: {
        url: baseUrl + 'hologramas052-list'
      },
      columns: [
  ...
  $(document).on('click', '.delete-record', function () {
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        $.ajax({
          type: 'DELETE',
          url: `${baseUrl}hologramas052-list/${id_solicitud}`,
          success: function () {
            dt_user.draw();
          },
...
$(document).on('click', '.pdfSolicitudHolograma', function () {
  ...
  const fv4 = FormValidation.formValidation(edit_activarHologramasForm, {
    ...
    $.ajax({
      url: '/solicitud_holograma052/update/updateActivar', // URL de la acción de actualización
      type: 'POST',
      data: formData,
      processData: false,
      contentType: false,
  ...
  $(document).on('click', '.edit-activos', function () {
    var id = $(this).data('id');
    $.get('/solicitud_holograma052/editActivados/' + id, function (data) {
      console.log(data);
      $('#edit_id').val(data.id);
