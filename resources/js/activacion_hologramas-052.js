...
$(function () {
  ...
  if (dt_user_table.length) {
    var dt_user = dt_user_table.DataTable({
      ...
      serverSide: true,
      ajax: {
        url: baseUrl + 'find_hologramas_activar052-list'
      },
      columns: [
  ...
  $(document).on('click', '.delete-record', function () {
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        $.ajax({
          type: 'DELETE',
          url: `${baseUrl}find_hologramas_activar052-list/${id_solicitud}`,
          success: function () {
            dt_user.draw();
          },
  ...
  const fv = FormValidation.formValidation(editHologramasForm, {
    ...
    $.ajax({
      url: '/solicitud_holograma052/update/', // URL de la ruta de actualización
      method: 'POST',
      data: formData,
      processData: false,
      contentType: false,
  ...
  const fv2 = FormValidation.formValidation(addPagoForm, {
    ...
    $.ajax({
      url: '/solicitud_holograma052/update2',
      type: 'POST',
      data: formData,
      processData: false,
      contentType: false,
  ...
  const fv3 = FormValidation.formValidation(addEnvioForm, {
    ...
    $.ajax({
      url: '/solicitud_holograma052/update3', // URL de la acción de actualización
      type: 'POST',
      data: formData,
      processData: false,
      contentType: false,
  ...
  $(document).on('click', '.edit-envio', function () {
    var id_solicitud = $(this).data('id');
    $.get('/solicitud_holograma052/edit/' + id_solicitud, function (data) {
  ...
  $('#asignarHologramaForm').submit(function (e) {
    ...
    $.ajax({
      url: '/solicitud_holograma052/updateAsignar',
      type: 'POST',
      data: formData,
      contentType: false,
      processData: false,
  ...
  const fv5 = FormValidation.formValidation(activarHologramasForm, {
    ...
    $.ajax({
      url: '/solicitud_holograma052/storeActivar', // Actualiza con la URL correcta
      type: 'POST',
      data: formData,
      processData: false,
      contentType: false,
  ...
  const fv4 = FormValidation.formValidation(edit_activarHologramasForm, {
    ...
    $.ajax({
      url: '/solicitud_holograma052/update/updateActivados', // URL de la acción de actualización
      type: 'POST',
      data: formData,
      processData: false,
      contentType: false,
  ...
  $(document).on('click', '.activos_hologramas', function () {
    var id = $(this).data('id');
    $.get('/solicitud_holograma052/editActivos/' + id, function (data) {
      $('#tablita').empty();
  ...
  $(document).on('click', '.edit-activos', function () {
    var id = $(this).data('id');
    $.get('/activacion_holograma052/edit/' + id, function (data) {
      console.log(data);
      $('#edit_id').val(data.id);
