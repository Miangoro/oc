        ...
        },
        {
          ...
          let acciones = '';
            ...
            if (window.puedeEliminarElUsuario) {
              acciones += `<a data-id="${full['id_marca']}" class="dropdown-item delete-record  waves-effect text-danger"><i class="ri-delete-bin-7-line ri-20px text-danger"></i> Eliminar marca</a>`;
            }
  ...
  $(document).on('click', '.edit-etiquetas', function () {
    ...
    $.get('/marcas-list/' + id_marca + '/editEtiquetas', function (data) {
      ...
      direccion.forEach(function (item) {
        direcciones +=
          "<option value='" + item.id_direccion + "'>" + item.destinatario + ' | ' + item.direccion + '</option>';
      });
      ...
      marca.sku.forEach(function (sku, index) {
        ...
        var documento_etiquetas = documentos.find(
          doc => doc.nombre_documento === 'Etiquetas' && (!doc.id_doc || doc.id_doc === id_unico)
        );
        var documento_corrugado = documentos.find(
          doc => doc.nombre_documento === 'Corrugado' && (!doc.id_doc || doc.id_doc === id_unico)
        );
...
`;
        ...
        $('#id_direccion' + index)
          ...
          .val(id_direccion)
          .trigger('change'); // Establecer el valor correcto
        $('#id_tipo' + index)
          .select2({
            dropdownParent: $('#etiquetas')
          })
