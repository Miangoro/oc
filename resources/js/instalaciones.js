  ...
  $(document).ready(function () {
    ...
    function toggleCamposCertificacion(certificacion) {
      if (certificacion === 'otro_organismo') {
        ...
        $('#edit_id_organismo')
          .val(instalacionData.id_organismo || '')
          .trigger('change');
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success) {
          ...
          $('#documentosTableBody').html(
            '<tr><td colspan="2" class="text-center">No se encontraron documentos.</td></tr>'
          );
        }
      },
      error: function () {
        $('#documentosTableBody').html(
          '<tr><td colspan="2" class="text-center">Error al cargar los documentos.</td></tr>'
        );
      }
    });
  }); */
