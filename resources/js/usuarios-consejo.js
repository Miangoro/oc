...
$(function () {
...

        {
          ...
          render: function (data, type, full, meta) {
            ...
            if (window.puedeEditarUsuario) {
              acciones += `<a data-id="${full['id']}" data-bs-toggle="offcanvas" data-bs-target="#offcanvasAddUser" href="javascript:;" class="dropdown-item edit-record"><i class="ri-edit-box-line ri-20px text-info"></i> Editar usuario consejo</a>`;
            }
            if (window.puedeEliminarUsuario) {
              acciones += `<a data-id="${full['id']}" class="dropdown-item delete-record  waves-effect text-danger"><i class="ri-delete-bin-7-line ri-20px text-danger"></i> Eliminar usuario consejo</a>`;
            }
  ...
  $(document).on('click', '.delete-record', function () {
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        $.ajax({
          type: 'DELETE',
          url: `${baseUrl}consejo-list/${user_id}`,
          success: function () {
            dt_user.draw();
          },
  ...
  $(document).on('click', '.edit-record', function () {
    ...
    // get data
    $.get(`${baseUrl}consejo-list\/${user_id}\/edit`, function (data) {
      $('#user_id').val(data.id);
